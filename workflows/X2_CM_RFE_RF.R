...
fs_workflow_description <- 'Univariate canonical correlation (X2) with 
                            Multivariate Correlation filter (MC) follow by
                            Recursive Feature Elimination (RFE) 
                            wrapped with Random Forest (RF).'

# assing workflow label (used to save plot, tables)
wf_label <- "X2_CM_RFE_RF"

# read original data
load('data/GSE48760_cardiac.rda')
...
# split large dataframe in smaller 5000-columns dfs
...
list.corr <- lapply(list.temp, function(x) linear.correlation(class ~., x))
# gain.info <- lapply(list.temp, function(x) information.gain(class ~., x))

# unlist and rename
weights <- rbind.fill(list.corr)
